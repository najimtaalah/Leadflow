...
import { getSessionExamenPratiqueById, getAffectationsBySession } from "@/lib/db/sessions";
...
import {
  ...
  RESULTAT_EXAMEN_LABELS, RESULTAT_EXAMEN_VARIANT,
} from "@/lib/format";
...
export default async function SessionExamenPratiqueApprenantsPage({ params }: Props) {
  const { id } = await params;
  const [session, affectations, user] = await Promise.all([
    getSessionExamenPratiqueById(id),
    getAffectationsBySession('pratique', id),
    getCurrentUser(),
  ]);
      ...
      <PageHeader
        title={
          <div className="flex items-center gap-2 min-w-0">
            <Link href="/sessions/examens-pratiques" className="text-foreground-muted hover:text-foreground transition-colors">
              <ChevronLeft className="h-4 w-4" />
            </Link>
            <span className="text-[14px] font-semibold text-foreground">Examen pratique — {formatDate(session.date_examen)}</span>
        ...
        actions={
          <div className="flex items-center gap-2">
            {canEdit && (
              <a href={`/sessions/examens-pratiques/${id}/resultats`} className="inline-flex items-center gap-1 text-[12px] px-3 py-1.5 rounded-[5px] border border-border bg-surface hover:bg-surface-hover transition-colors">
                Saisir les résultats
              </a>
            )}
            {canEdit && placesRestantes > 0 && <AffecterDialog sessionType="pratique" sessionId={id} />}
          </div>
        }
      />
      <PageContent>
          ...
          <table className="w-full text-[12px] border-collapse">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="text-left px-4 py-2 text-[11px] font-medium text-foreground-muted">Résultat théorique</th>
                <th className="text-left px-4 py-2 text-[11px] font-medium text-foreground-muted">Résultat pratique</th>
            ...
            <tbody>
              {affectations.map((a) => (
                <tr key={a.id} className="border-b border-border hover:bg-surface-hover transition-colors">
                  ...
                  <td className="px-4 py-2">
                    {a.resultat_theorique ? (
                      <Badge variant={RESULTAT_EXAMEN_VARIANT[a.resultat_theorique] as Parameters<typeof Badge>[0]['variant']}>
                        {RESULTAT_EXAMEN_LABELS[a.resultat_theorique]}
                      </Badge>
                    ) : <span className="text-foreground-subtle">—</span>}
                  </td>
                  <td className="px-4 py-2">
                    {a.resultat_pratique ? (
                      <Badge variant={RESULTAT_EXAMEN_VARIANT[a.resultat_pratique] as Parameters<typeof Badge>[0]['variant']}>
                        {RESULTAT_EXAMEN_LABELS[a.resultat_pratique]}
                      </Badge>
                    ) : <span className="text-foreground-subtle">—</span>}
                  </td>
              ...
              {affectations.length === 0 && (
                <tr><td colSpan={6} className="px-4 py-8 text-center text-foreground-muted">Aucun apprenant affecté à cet examen pratique</td></tr>
              )}
            </tbody>
          </table>
        </div>
