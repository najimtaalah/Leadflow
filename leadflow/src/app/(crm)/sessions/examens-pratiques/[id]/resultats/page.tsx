...
import { getSessionExamenPratiqueById } from "@/lib/db/sessions";
...
import { ResultatsPratiqueClient } from "./resultats-client";

interface Props {
  params: Promise<{ id: string }>;
}

export default async function SessionExamenPratiqueResultatsPage({ params }: Props) {
  const { id } = await params;
  const [session, affectations, user] = await Promise.all([
    getSessionExamenPratiqueById(id),
    getAffectationsSessionWithTentative('pratique', id),
    getCurrentUser(),
  ]);
      ...
      <PageHeader
        title={
          <div className="flex items-center gap-2 min-w-0">
            <Link href={`/sessions/examens-pratiques/${id}/apprenants`} className="text-foreground-muted hover:text-foreground transition-colors">
              <ChevronLeft className="h-4 w-4" />
            </Link>
            <span className="text-[14px] font-semibold text-foreground">Résultats — Examen pratique {formatDate(session.date_examen)}</span>
