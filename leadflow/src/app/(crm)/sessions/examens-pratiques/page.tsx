...
import { getSessionsExamensPratiques } from "@/lib/db/sessions";
...
import { CreateSessionExamenPratiqueDialog } from "./create-dialog";

export default async function SessionsExamensPratiquesPage() {
  const user = await getCurrentUser();
  const sessions = await getSessionsExamensPratiques();
  ...
  return (
    <>
      <PageHeader
        ...
        actions={canCreate ? <CreateSessionExamenPratiqueDialog /> : undefined}
      />
      <PageContent>
        <div className="overflow-x-auto">
          <table className="w-full text-[12px] border-collapse">
            ...
            <tbody>
              {sessions.map((s) => (
                <tr key={s.id} className="border-b border-border hover:bg-surface-hover transition-colors">
                  <td className="px-4 py-2">
                    <Link href={`/sessions/examens-pratiques/${s.id}/apprenants`} className="font-medium text-foreground hover:text-accent tabular-nums">
              ...
              {sessions.length === 0 && (
                <tr><td colSpan={5} className="px-4 py-8 text-center text-foreground-muted">Aucune session d&apos;examen pratique</td></tr>
              )}
            </tbody>
          </table>
        </div>
