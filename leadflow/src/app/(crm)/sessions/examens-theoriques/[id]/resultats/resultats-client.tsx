...
export function ResultatsTheoriqueClient({ sessionId, affectations, canSaisir, isSuperAdmin, sessionClosed }: Props) {
  ...
  if (affectations.length === 0) {
    return (
      <div className="px-4 py-8 text-center text-foreground-muted text-[12px]">
        Aucun candidat affecté à cet examen théorique
      </div>
    );
  }
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-[12px] border-collapse">
        ...
        <tbody>
          {affectations.map((a) => {
            const row = rows[a.dossier_id];
            const isEditable = canSaisir && (isSuperAdmin || !sessionClosed) && !!a.tentative_id;
