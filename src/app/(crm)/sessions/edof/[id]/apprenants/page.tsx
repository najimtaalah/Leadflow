...
import { getSessionEdofById, getAffectationsBySession } from "@/lib/db/sessions";
import { getCurrentUser } from "@/lib/auth";
import { PageHeader, PageContent } from "@/components/ds/app-layout";
import { Badge } from "@/components/ui/badge";
import {
  formatDate, SESSION_EDOF_STATUT_LABELS, SESSION_EDOF_STATUT_VARIANT,
...
export default async function SessionEdofApprenantsPage({ params }: Props) {
  const { id } = await params;
  const [session, affectations, user] = await Promise.all([
    getSessionEdofById(id),
    getAffectationsBySession('edof', id),
    getCurrentUser(),
  ]);
      ...
      <PageHeader
        title={
          <div className="flex items-center gap-2 min-w-0">
            <Link href="/sessions/edof" className="text-foreground-muted hover:text-foreground transition-colors">
              <ChevronLeft className="h-4 w-4" />
            </Link>
            <span className="text-[14px] font-semibold text-foreground truncate font-mono">
              {session.reference_edof ?? 'Session EDOF'}
            </span>
            <Badge variant={SESSION_EDOF_STATUT_VARIANT[session.statut] as Parameters<typeof Badge>[0]['variant']}>
              {SESSION_EDOF_STATUT_LABELS[session.statut]}
            </Badge>
          </div>
        }
        description={
          <span className="flex items-center gap-3 text-[12px] text-foreground-muted">
            ...
            {session.date_fin_cpf && <span>Fin CPF : {formatDate(session.date_fin_cpf)}</span>}
            <span className="flex items-center gap-1"><Users className="h-3 w-3" />{affectations.length}/{session.capacite_max}</span>
          </span>
        }
        actions={canEdit && placesRestantes > 0 ? <AffecterDialog sessionType="edof" sessionId={id} /> : undefined}
      />
      <PageContent>
          ...
          <table className="w-full text-[12px] border-collapse">
            ...
            <tbody>
              ...
              {affectations.length === 0 && (
                <tr><td colSpan={5} className="px-4 py-8 text-center text-foreground-muted">Aucun apprenant affecté à cette session EDOF</td></tr>
              )}
            </tbody>
          </table>
        </div>
