...
import { getSessionExamenTheoriqueById, getAffectationsBySession } from "@/lib/db/sessions";
...
export default async function SessionExamenTheoriqueApprenantsPage({ params }: Props) {
  const { id } = await params;
  const [session, affectations, user] = await Promise.all([
    getSessionExamenTheoriqueById(id),
    getAffectationsBySession('theorique', id),
    getCurrentUser(),
  ]);
      ...
      <PageHeader
        title={
          <div className="flex items-center gap-2 min-w-0">
            <Link href="/sessions/examens-theoriques" className="text-foreground-muted hover:text-foreground transition-colors">
              <ChevronLeft className="h-4 w-4" />
            </Link>
            <span className="text-[14px] font-semibold text-foreground">Examen théorique — {formatDate(session.date_examen)}</span>
          ...
          <table className="w-full text-[12px] border-collapse">
            ...
            <tbody>
              ...
              {affectations.length === 0 && (
                <tr><td colSpan={5} className="px-4 py-8 text-center text-foreground-muted">Aucun apprenant affecté à cet examen théorique</td></tr>
              )}
            </tbody>
          </table>
        </div>
